...
bapa=r"D:\POLYU_dissertation\July_2021_useful\July_clustering\cluster_csv\HK_cluster_csv\HK.csv"
df = pd.read_csv(bapa, index_col=0, header=0)
print(df.head())

ne_arr = np.zeros((31, 288))
name_arr = np.zeros(31, dtype="U25")
name_arr = np.array(df.index.to_list())
ne_arr = df.to_numpy(dtype="float32")

# print(name_arr)
day_arr = np.array([iw.day_reco(per_day=x) for x in name_arr])
# print(day_arr)
print(ne_arr.shape)
# p10_l300_e12_r30
k_arr = ne_arr
...
def tskm(Y):
    ...
    for i in range(1,Y):
        ...
        print("centroids: ")
        print(centroids)
        result_df["date"] = name_arr
        result_df["day"] = day_arr
        co_na = "n="+str(i)
        result_df[co_na] = np.array(labels)
    ne_na = "tskm_avedays_2_" + str(Y) + ".csv"
...
plt.title("K-means clustering (daily pattern)", size=40)
plt.tick_params(which='major', length=5, width=0, labelsize=15)
plt.subplots_adjust(bottom=0.2) # to solve the legend can't show entirely
plt.legend(
        loc="upper left", bbox_to_anchor=(0.01, -0.1), ncol=2, frameon=False, handleheight=0.3,markerscale=3,
        handlelength=0, fontsize=20, title_fontsize=8, labelspacing=0.3, borderpad=0.0, columnspacing=15)
ne_na = "tskm_aveday"+"_k"+str(k)+"_p" + str(perplexity) + "_l" + str(learning_rate) + \
